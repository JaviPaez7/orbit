import { z } from 'zod';
import { COMPLETED_STATUSES, ISSUE_PRIORITIES, ISSUE_PRIORITY_WEIGHT, ISSUE_STATUSES, } from './constants';
/* -------------------------------------------------------------------------- */
...
export function deriveWorkspaceKey(name) {
    const words = name
        .split(/[^A-Za-z0-9]+/)
        .filter(Boolean)
        .map((w) => w.toUpperCase());
...
export function mentionHandle(name, email) {
...
export const ISSUE_PRIORITY_ORDER = [...ISSUE_PRIORITIES].reverse();
