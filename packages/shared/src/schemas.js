import { z } from 'zod';
import { ACTIVITY_ACTIONS, CYCLE_STATUSES, ESTIMATE_OPTIONS, ISSUE_PRIORITIES, ISSUE_RELATION_TYPES, ISSUE_STATUSES, NOTIFICATION_TYPES, PROJECT_ICONS, PROJECT_STATUSES, WORKSPACE_ROLES, } from './constants';
/* -------------------------------------------------------------------------- */
/*  Primitives                                                                */
/* -------------------------------------------------------------------------- */
...
export const createIssueSchema = z.object({
    ...
    estimate: z
        ...
        .optional()
        .refine((v) => v === undefined || v === null || ESTIMATE_OPTIONS.includes(v), 'Invalid estimate'),
 ...
 */
function csvArray(item, max = 100) {
    return z.preprocess((value) => {
        if (value === undefined || value === null || value === '')
            return undefined;
        if (Array.isArray(value)) {
            return value.flatMap((entry) => typeof entry === 'string' && entry.includes(',') ? entry.split(',').filter(Boolean) : [entry]);
        }
...
export const bulkIssueSchema = z
    ...
    (v.removeLabelIds?.length ?? 0) > 0, 'Choose at least one change');
