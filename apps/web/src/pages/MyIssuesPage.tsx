...
export default function MyIssuesPage() {
  ...
  const created = issues.filter(
    (issue) => issue.creatorId === user?.id && issue.assigneeId !== user?.id,
  );
  const openAssigned = assigned.filter(
      ...
      <Header
        ...
        filters={
          <span className="text-2xs text-subtle">
            {openAssigned.length} open assigned · {assigned.length} total assigned ·{' '}
            {created.length} reported
          </span>
        }
      />
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {!query.isLoading && issues.length > 0 && (
          <div className="flex items-center gap-3 px-3 py-2 text-2xs text-subtle">
            ...
            <span>
              Tip: press <span className="kbd">/</span> to search, or <span className="kbd">C</span>{' '}
              to create an issue.
            </span>
          </div>
        )}
