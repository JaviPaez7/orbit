      ...
      trigger={(triggerProps) => (
        <button
          ...
          className={cn(
            ...
            compact
              ? 'px-1.5 py-0.5 text-xs hover:bg-hover'
              : 'w-full border border-line bg-surface px-2 py-1 hover:bg-hover',
            className,
          )}
          {...triggerProps}
          onClick={(event) => {
        ...
        >
          {trigger ?? (
            <>
              ...
              {selected?.icon && (
                <span className="[&>svg]:h-3.5 [&>svg]:w-3.5">{selected.icon}</span>
              )}
              {selected?.avatar && (
  ...
  return (
    ...
    >
      <div className="max-h-72 overflow-y-auto">
        ...
        {options.length === 0 && (
          <p className="px-2 py-3 text-center text-xs text-subtle">Nothing to choose</p>
        )}
        {options.map((option) => {
